...
import BatchNorm from "./BatchNorm.jsx";
import AddBatchNormButton from "./AddBatchNormButton.jsx";
...
export default function Workspace() {
  ...
  const nodeTypes = React.useMemo(() => ({
    ...
    batchnorm: (props) => <BatchNorm {...props} setNodes={setNodes} />,
    //batchnorm2d: (props) => <BatchNorm {...props} setNodes={setNodes} />,
...
useEffect(() => {
  ...
  const handleTrain = async () =>{
    ...
    if(inputSource === "dataset"){
      try{
        setTrainingStatus("Training on dataset...");
        setConsoleOutput([]);
  ...
  const handleRun = async () =>{
    ...
    if(inputSource === "dataset"){
      ...
      try{
        ...
        setTrainingStatus("Testing completed.");
      } catch (err){
    ...
    <div style={{ width: "100vw", height: "100vh", background: "linear-gradient(160deg, #d8eef7, #c7e6f4, #bfe0f0)"}}>
      ...
      <div 
        ...
        {open === "core" && (
          <div style={{
            ...
            <button onClick={handleAddLinear}>➕ Linear</button>
            <button onClick={handleAddEmbedding}>➕ Embedding</button>
          </div>
        )}
      </div>
      ...
      {open === "activation" && (
        <div style={{
          ...
          <button onClick={handleAddReLU}>➕ ReLU</button>
          <button onClick={() => handleAddSoftmaxNode("softmax")}>➕ Softmax</button>
        </div>
      )}
    </div>
    ...
    <div style={{ position: "relative" }}>
      ...
      {open === "norm" && (
        <div style={{
          ...
          <button onClick={handleAddLayerNorm}>➕ LayerNorm</button>
          <AddBatchNormButton onAdd={handleAddBatchNorm} />
        </div>
      )}
    </div>
    ...
    <div style={{ position: "relative" }}>
      ...
      {open === "tensor" && (
        <div style={{
          ...
          <button 
          onClick={() => handleAddConcatNode("concat")}>
          ➕Concat
        </button>
        ...
        <button 
        onClick={() => handleAddMatMulNode("matmul")}>
          ➕MatMul
        </button>
        <button 
        onClick={() => handleAddScaleNode("scale")}>
          ➕Scale
        </button>
        <button
        onClick={() => handleAddMaskNode("mask")}>
          ➕Mask
        </button>
        </div>
      )}
